...
class CartNotifier extends ChangeNotifier {
  ...
  void updateProduct({
    ...
    cartItemList[index].unitPrice = price; // Always store base price
    cartItemList[index].quantity =
        num.tryParse(qty) ?? 0; // Always store base quantity
    if (displayUnit != null) {
      cartItemList[index].displayUnit = displayUnit;
    }
    if (displayQuantity != null) {
      cartItemList[index].displayQuantity = displayQuantity;
    }
    if (displayPrice != null) {
      cartItemList[index].displayPrice = displayPrice;
    }
    calculatePrice(); // This uses base quantity and base price, so total stays correct
  }
