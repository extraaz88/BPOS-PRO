...
class DemoPrintScreen2 extends ConsumerStatefulWidget {
  const DemoPrintScreen2({super.key});

  @override
  ConsumerState<DemoPrintScreen2> createState() => _DemoPrintScreenState();
}

class _DemoPrintScreenState extends ConsumerState<DemoPrintScreen2> {
  ...
  Future<List<int>> _generatePrintBytes() async {
    ...
    try {
      ...
      // Initialize printer - but immediately set position to avoid spacing
      bytes += Uint8List.fromList([0x1B, 0x40]); // Initialize printer (ESC @)
      
      // IMMEDIATELY set absolute vertical position to 0 to prevent any top spacing
      // This must come right after initialization
      bytes += Uint8List.fromList([0x1B, 0x64, 0x00]); // Set absolute vertical position to 0 (ESC d 0)
      
      // Remove ALL spacing - aggressive approach
      ...
      // 8. Disable automatic paper feed after printing
      bytes += Uint8List.fromList([0x1D, 0x50, 0x00, 0x00]); // No automatic feed (GS P)
      
      // Print 4 copies without any gap between them
      const int numberOfCopies = 4;
      ...
      for (int i = 0; i < numberOfCopies; i++) {
        ...
        // Generate barcode with 0 spacing - no extra spacing after barcode
        ...
        // Absolutely NO spacing between copies
        // Don't add any feed commands between copies
        // The next copy will start immediately after barcode
      }
      
      // Use partial cut (GS V 0) instead of full cut to avoid extra spacing
      // Partial cut doesn't feed extra paper
      bytes += Uint8List.fromList([0x1D, 0x56, 0x00]); // Partial cut (GS V 0) - no feed
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Click to print 4 copies of "hello extraaz"\nwith zero spacing and barcode (no gap between copies)',
