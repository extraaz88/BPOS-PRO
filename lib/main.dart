...
import 'package:mobile_pos/Screens/Authentication/RegisterFlow/register_phone_screen.dart';
...
void main() async {
  ...
  if (!kIsWeb && (Platform.isWindows || Platform.isLinux || Platform.isMacOS)) {
    try {
      ...
      print(
          '⚠️ Running on desktop - ensure sqflite_common_ffi is configured if using database');
    } catch (e) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Locale>(
      ...
      builder: (context, snapshot) {
        // Use saved locale or default to English
        final initialLocale = snapshot.data ?? const Locale('en');

        return pro.ChangeNotifierProvider<LanguageChangeProvider>(
          create: (context) =>
              LanguageChangeProvider(initialLocale: initialLocale),
                ...
                builder: (context) {
                  return MaterialApp(
                    ...
                    routes: {
                      ...
                      '/signup': (context) => const RegisterPhoneScreen(),
                      ...
                      '/Stock': (context) =>
                          const StockList(isFromReport: false),
  ...
  Future<Locale> _getInitialLocale() async {
    try {
      ...
      // Update the global selectedLanguage variable for printing
