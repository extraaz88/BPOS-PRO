                    ...
                    expenseData.when(data: (mainData) {
                      if (mainData.isNotEmpty) {
                        ...
                        for (var element in mainData) {
                          if ((fromDate.isBefore(DateTime.parse(element.expenseDate?.substring(0, 10) ?? '')) || DateTime.parse(element.expenseDate?.substring(0, 10) ?? '').isAtSameMomentAs(fromDate)) && (toDate.isAfter(DateTime.parse(element.expenseDate?.substring(0, 10) ?? '')) || DateTime.parse(element.expenseDate?.substring(0, 10) ?? '').isAtSameMomentAs(DateTime.parse(toDate.toString().substring(0, 10))))) {
                        ...
                        return SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return Visibility(
                                visible: (fromDate.isBefore(DateTime.parse(mainData[index].expenseDate ?? '')) || DateTime.parse(mainData[index].expenseDate?.substring(0, 10) ?? '').isAtSameMomentAs(fromDate)) && (toDate.isAfter(DateTime.parse(mainData[index].expenseDate ?? '')) || DateTime.parse(mainData[index].expenseDate?.substring(0, 10) ?? '').isAtSameMomentAs(DateTime.parse(toDate.toString().substring(0, 10)))),
                                child: Column(
                                  children: [
                                    Padding(
                                      padding: const EdgeInsets.all(10.0),
