...
class _BarcodeGeneratorScreenState extends State<BarcodeGeneratorScreen> {
  ...
  /// Generate barcode PDF with 30mm x 50mm label size
  Future<Uint8List?> _generatePdfBytes() async {
    final pdf = pw.Document();
    List<pw.Widget> barcodeWidgets = [];

    for (var selectedProduct in selectedProducts) {
      final String? code = selectedProduct.product.productCode;
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                if (showPrice)
                  pw.Text(
                    '₹${selectedProduct.product.productSalePrice ?? ''}',
                    style: const pw.TextStyle(fontSize: 8),
                  ),
              ],
            ),
    ...
    if (barcodeWidgets.isEmpty) return null;
  ...
  Future<void> _printToThermal(WidgetRef ref) async {
    try {
      ...
      final hasValid = payload.any((p) => (p['code'] ?? '').toString().trim().isNotEmpty);
      if (!hasValid) {
      ...
      final bool success = await ref.read(printerPurchaseProviderNotifier).printBarcodeProductsAsImage(
            products: payload,
            businessName: '',
          );
                      ...
                      children: [
                        ...
                        TypeAheadField<ProductModel>(
                          ...
                          suggestionsCallback: (pattern) {
                            return products
                                .where((product) =>
                                    product.productName!.toLowerCase().startsWith(pattern.toLowerCase()))
                                .toList();
                          },
                          itemBuilder: (context, suggestion) => ListTile(
                            title: Text(suggestion.productName ?? ''),
                            subtitle: Text('Code: ${suggestion.productCode ?? ''}'),
                            trailing: Text('₹${suggestion.productSalePrice ?? 0}'),
                          ),
                        ...
                        Row(
                          children: [
                            Checkbox(
                              ...
                              value: showCode,
                              onChanged: (v) => _toggleCheckbox(v!, (val) => showCode = val),
                            ),
                            Text(lang.S.of(context).showCode),
                            Checkbox(
                              activeColor: kMainColor,
                              value: showPrice,
                              onChanged: (v) => _toggleCheckbox(v!, (val) => showPrice = val),
                            ),
                            Text(lang.S.of(context).showPrice),
                            Checkbox(
                              activeColor: kMainColor,
                              value: showName,
                              onChanged: (v) => _toggleCheckbox(v!, (val) => showName = val),
                            ),
                        ...
                        selectedProducts.isNotEmpty
                                    ...
                                    return DataRow(
                                      cells: [
                                        DataCell(Column(
                                          ...
                                          children: [
                                            Text(selectedProduct.product.productName ?? 'N/A'),
                                            Text(selectedProduct.product.productCode ?? '',
                                                style: const TextStyle(color: Colors.grey)),
                                          ],
                                        )),
                                        DataCell(Text(selectedProduct.product.productStock?.toString() ?? '0')),
                                        DataCell(TextFormField(
                                          ...
                                          onChanged: (v) {
                                            setState(() {
                                              selectedProduct.quantity = int.tryParse(v) ?? 1;
                                            });
                                          },
                                        ...
                                        DataCell(IconButton(
                                          icon: const Icon(Icons.delete, color: kMainColor),
                                          onPressed: () {
                                            setState(() => selectedProducts.remove(selectedProduct));
                                          },
                                        )),
                                      ],
                                    );
                            ...
                            : const Center(
                                child: Column(
                                  children: [
                                    SizedBox(height: 50),
                                    Icon(IconlyLight.document, color: kMainColor, size: 70),
                                    Text("No items selected", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                                  ],
                                ),
                              ),
                      ],
                        ...
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () async {
                            if (selectedProducts.isNotEmpty) {
                              ...
                              _showSnackBar(lang.S.of(context).noProductSelected);
                            }
                          },
