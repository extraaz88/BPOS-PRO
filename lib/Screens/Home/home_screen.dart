                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Visibility(
                        visible:
                            details.user?.visibility?.dashboardPermission ??
                                true,
                        ...
                        child: Container(
                          ...
                          child: ListTile(
                            ...
                            title: RichText(
                              text: TextSpan(
                                text:
                                    '${details.enrolledPlan?.plan?.subscriptionName ?? 'No Active'} ${lang.S.of(context).package} ',
                                style: theme.textTheme.bodyLarge
                                    ?.copyWith(fontWeight: FontWeight.w600),
                              ),
                            ),
                            subtitle: Text(getDayLeftInExpiring(
  ...
  bool checkPermission({required String item}) {
    if (item == 'Sales' && (widget.visibility?.salePermission ?? true)) {
      return true;
        ...
        (widget.visibility?.partiesPermission ?? true)) {
      return true;
    } else if (item == 'Purchase' &&
        (widget.visibility?.purchasePermission ?? true)) {
      return true;
    } else if (item == 'Products' &&
        (widget.visibility?.productPermission ?? true)) {
      return true;
    } else if (item == 'Due List' &&
        (widget.visibility?.dueListPermission ?? true)) {
      return true;
    } else if (item == 'Stock' &&
        (widget.visibility?.stockPermission ?? true)) {
      return true;
    } else if (item == 'Reports' &&
        (widget.visibility?.reportsPermission ?? true)) {
      return true;
    } else if (item == 'Sales List' &&
        (widget.visibility?.salesListPermission ?? true)) {
      return true;
    } else if (item == 'Purchase List' &&
        (widget.visibility?.purchaseListPermission ?? true)) {
      return true;
    } else if (item == 'Loss/Profit' &&
        (widget.visibility?.lossProfitPermission ?? true)) {
      return true;
    } else if (item == 'Expense' &&
        (widget.visibility?.addExpensePermission ?? true)) {
      return true;
    } else if (item == 'Income' &&
        (widget.visibility?.addIncomePermission ?? true)) {
      return true;
    } else if (item == 'tax') {
      return true;
            ...
            children: [
              ...
              Flexible(
                ...
                style: Theme.of(context)
                    .textTheme
                    .bodyLarge
                    ?.copyWith(color: DAppColors.kNeutral700),
                overflow: TextOverflow.ellipsis,
                maxLines: 1,
              ))
            ],
...
String getDayLeftInExpiring(
  ...
  if (expiringDay.difference(DateTime.now()).inDays < 1) {
    return shortMSG
        ...
        : '${expiringDay.difference(DateTime.now()).inHours} Hours Left';
  } else {
    return shortMSG
