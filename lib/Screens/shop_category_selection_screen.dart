...
class ShopCategorySelectionScreen extends StatefulWidget {
  ...
  const ShopCategorySelectionScreen({Key? key, this.preselectedCategoryName})
      : super(key: key);

  @override
  State<ShopCategorySelectionScreen> createState() =>
      _ShopCategorySelectionScreenState();
}

class _ShopCategorySelectionScreenState
    extends State<ShopCategorySelectionScreen> {
  ...
  void initState() {
    ...
    print(
        'ShopCategorySelectionScreen - Preselected Category: $_selectedCategoryName');
        ...
        Future.delayed(const Duration(milliseconds: 800), () {
          if (mounted) {
            ...
            try {
              ...
              print(
                  'Found category with exact match: ${foundCategory['name']}');
            } catch (e) {
              ...
              try {
                ...
                print(
                    'Found category with case-insensitive match: ${foundCategory['name']}');
              } catch (e2) {
                print('Category not found: $_selectedCategoryName');
                print(
                    'Available categories: ${shopCategories.map((c) => c['name']).toList()}');
                // If not found, don't auto-select - let user choose
                return;
              }
            }

            selectShopCategory(foundCategory['name'],
                List<String>.from(foundCategory['features']));
          }
        });
      });
    }
  ...
  Future<void> selectShopCategory(
      String categoryName, List<String> features) async {
