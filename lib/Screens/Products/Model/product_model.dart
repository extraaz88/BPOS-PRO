class ProductModel {
  ...
  ProductModel.fromJson(dynamic json) {
    ...
    // Handle status field - can be boolean or string
    final statusRaw = json['product_status'] ?? json['productStatus'] ?? json['status'];
    if (statusRaw != null) {
      if (statusRaw is bool) {
        productStatus = statusRaw ? 'active' : 'inactive';
      } else if (statusRaw is num) {
        productStatus = statusRaw == 1 ? 'active' : 'inactive';
      } else {
        ...
        if (['true', '1', 'active', 'yes', 'enabled'].contains(s)) {
          ...
          productStatus = s.isNotEmpty ? s : null;
        }
      }
    }
