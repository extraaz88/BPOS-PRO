...
class SalesInvoiceDetails extends StatefulWidget {
  ...
  final bool? fromSale;
  final List<AddToCartModel>? cartItems; // For displaying selected units
...
class _SalesInvoiceDetailsState extends State<SalesInvoiceDetails> {
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _gstNumber = prefs.getString("gst") ?? "";
      _ifscCode = prefs.getString("ifsc") ?? "";
    });
  }
  ...
  String _getQuantityWithUnit(SalesDetails saleDetail, num quantity) {
    // Try to get display unit from cart items if available
    if (widget.cartItems != null) {
      ...
      if (cartItem.productId != -1) {
        if (cartItem.displayQuantity != null && cartItem.displayUnit != null) {
          return '${mainConstant.formatPointNumber(cartItem.displayQuantity!)} ${cartItem.displayUnit}';
        }
        if (cartItem.unitName != null && cartItem.unitName!.isNotEmpty) {
          return '${mainConstant.formatPointNumber(quantity)} ${cartItem.unitName}';
        }
      }
    }

    // Fallback to just quantity
    return mainConstant.formatPointNumber(quantity);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final num totalAmountWithReturns = totalReturnAmount + storedRoundedAmount;
  ...
  Future<void> _printWithSelectedLanguage(
    ...
    try {
      ...
      // Print the invoice (removed delay for faster printing)
