...
class ProductAddToCartFormState extends State<SalesAddToCartForm> {
  ...
  String _baseUnit = '';
  List<_UnitOption> _unitOptions = const [];
  ...
  void initState() {
    ...
    _baseUnit = (widget.batchWiseStockModel.unitName ?? '').toLowerCase();
    _unitOptions = _buildUnitOptions(_baseUnit);
    _selectedDisplayUnit =
        _unitOptions.isNotEmpty ? _unitOptions.first.code : _baseUnit;
              ...
              Padding(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  items: _unitOptions
                      ...
                      .toList(),
                  onChanged: (value) => _onSellUnitChanged(value),
                ),
              ),
            // Stock quantity
            Row(
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap: () async {
                  ...
                  final displayQty = _toDouble(productQuantityController.text);
                  final displayPrice = _toDouble(salePriceController.text);
                  final factor = _unitFactorFor(_selectedDisplayUnit);
                  final qtyBase = displayQty * factor;
                  final priceBase =
                      factor == 0 ? displayPrice : displayPrice / factor;

                  ref.watch(cartNotifier).updateProduct(
                        productId: widget.batchWiseStockModel.productId,
                        price:
                            priceBase.toString(), // Base price for calculations
                        qty: qtyBase
                            .toString(), // Base quantity for calculations
                        variantKey: widget.batchWiseStockModel.variantKey,
                        displayUnit: _selectedDisplayUnit,
                        displayQuantity: displayQty,
                        displayPrice:
                            displayPrice, // Price per display unit for display
                      );
  ...
  void _onSellUnitChanged(String? newUnit) {
    if (newUnit == null || newUnit == _selectedDisplayUnit) return;

    final currentDisplayQty = _toDouble(productQuantityController.text);
    final currentDisplayPrice = _toDouble(salePriceController.text);
    final currentFactor = _unitFactorFor(_selectedDisplayUnit);

    // Convert current display values to base, then to the new display unit
    final baseQty = currentDisplayQty * currentFactor;
    final basePrice = currentFactor == 0
        ? currentDisplayPrice
        : currentDisplayPrice / currentFactor;
    final newFactor = _unitFactorFor(newUnit);

    setState(() {
      _selectedDisplayUnit = newUnit;
      productQuantityController.text =
          formatPointNumber(baseQty / newFactor); // show in new unit
      salePriceController.text = formatPointNumber(newFactor == 0
          ? basePrice
          : basePrice * newFactor); // price per new unit
    });
  }

  double _unitFactorFor(String? unitCode) {
    if (unitCode == null) return 1;
    return _unitOptions
        .firstWhere(
          (u) => u.code == unitCode,
          orElse: () => _UnitOption(code: unitCode, factorToBase: 1),
        )
        .factorToBase;
  }
  ...
  List<_UnitOption> _buildUnitOptions(String baseUnit) {
    switch (baseUnit) {
      case 'kg':
        return [
          _UnitOption(code: 'kg', label: 'Kilogram', factorToBase: 1),
          _UnitOption(code: 'g', label: 'Gram', factorToBase: 0.001),
        ];
      case 'g':
        return [
          _UnitOption(code: 'g', label: 'Gram', factorToBase: 1),
          _UnitOption(code: 'kg', label: 'Kilogram', factorToBase: 1000),
        ];
      case 'l':
      case 'liter':
      case 'litre':
        return [
          _UnitOption(code: 'l', label: 'Liter', factorToBase: 1),
          _UnitOption(code: 'ml', label: 'Milliliter', factorToBase: 0.001),
        ];
      case 'ml':
      case 'milliliter':
      case 'millilitre':
        return [
          _UnitOption(code: 'ml', label: 'Milliliter', factorToBase: 1),
          _UnitOption(code: 'l', label: 'Liter', factorToBase: 1000),
        ];
      default:
        final label = baseUnit.isEmpty ? 'Unit' : baseUnit;
        final code = baseUnit.isEmpty ? 'unit' : baseUnit;
        return [_UnitOption(code: code, label: label, factorToBase: 1)];
    }
  }
