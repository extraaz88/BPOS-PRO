...
class _SaleBillSummaryScreenState extends ConsumerState<SaleBillSummaryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final paymentTypes =
        paymentTypesAsync.asData?.value ?? <PaymentTypeModel>[];
    ...
    final totalTaxAmount = args.cartSaleProducts.fold<double>(
      0,
      (previousValue, element) => previousValue + (element.taxAmount ?? 0.0),
    );
    ...
    final num roundingDifference =
        shouldDisplayRoundOff ? args.totalPayableAmount - baseTotalAmount : 0;
    ...
    try {
      ...
      Navigator.pop(
        ...
        SaleSummaryResult(
          ...
          cartItems: args.cartItems, // Pass cart items for invoice display
        ),
      );
    } catch (e) {
      EasyLoading.dismiss();
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          '${_labels.quantity}: ${item.displayQuantity ?? item.quantity}${item.displayUnit != null ? ' ${item.displayUnit}' : ''} × ${_formatCurrency(item.displayPrice ?? unitPrice)}',
